...
モバイルの計算能力とデータ速度が急増する中、我々は初期段階のSolana Saga Mobileを通じて _D.A.G.G.E.R._ のパーミッションレス・モバイル・コンピュートとストレージを開発しています。この機能強化は、モバイルアプリでShdwDriveを使用するビルダーの既存のエコシステムをサポートします。 _D.A.G.G.E.R._ モバイルは、AIやゲームへの需要が高まる中、分散型台帳技術をスマートフォンに導入する予定です。ShdwDriveとモバイル実装を含む _D.A.G.G.E.R._ を、当社のロードマップに沿って、モバイルとエッジAIソリューションの実用的な選択肢にすることを目指しています。
